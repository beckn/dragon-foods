                            ...
                            if (order.items) {
                                ...
                                items.forEach(item => {
                                    if (!uniqueItemIds.has(item.id)) {
                                        ...
                                        let mediaUrl = '';
                                        if (item?.descriptor && item?.descriptor?.media && item.descriptor.media.length > 0) {
                ...
                <>
                    ...
                    {story.map((item, index) => {
                        ...
                        if (mediaUrl.endsWith(".mp3")) {
                            // MP3 audio
                            return <Box pt={2} pb={20}><AudioPlayer key={index} mediaUrl={mediaUrl} /></Box>;
                        } else if (mediaUrl.endsWith(".pdf")) {
                            // PDF document
                            return <Box pt={2} pb={20}><PDFViewer key={index} src={mediaUrl} /></Box>;
                        } else if (mediaUrl.includes("youtube.com") || mediaUrl.includes("youtu.be")) {
                            // YouTube video
                            return (<Box pt={2} pb={20}><YouTubeEmbed key={index} url={mediaUrl} /></Box>);
                        } else if (mediaUrl.endsWith(".mp4")) {
                            // MP4 video
                            return <Box pt={2} pb={20}><VideoPlayer key={index} url={mediaUrl} /></Box>;
                        } else {
                            // External website content
                            return <Box pt={2} pb={20}><ExternalLink key={index} url={mediaUrl} /></Box>;
                        }
                    })}
                </>
            )}
